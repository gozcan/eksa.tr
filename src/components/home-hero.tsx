...
function Header({
  ...
  messages,
  theme,
}: HomeHeroProps & { theme: HeaderTheme }) {
  const [menuOpen, setMenuOpen] = useState(false);
  const navItems = [
    { href: `/${locale}#vision`, label: messages.nav.vision },
  ...
  // Which sections need a light (dark-text) navbar.
  const lightThemeSections = new Set(['projects', 'about', 'contact']);

  // For overlayFromPrevious pinned sections the anchor sits exactly at the
  // "enter complete" scroll position (triggerStart = anchorTop - 1vh,
  // enter length = 1vh → enter done at anchorTop).  Content is revealed
  // during the PIN phase via buildPinnedTimeline.  We calculate how many
  // viewport heights past the anchor puts us at pin ≈ 95 % so content is
  // fully visible without relying on snap.
  //
  //   services  enterLen=1 pinLen=2.6 total=3.6 enterRatio≈0.278
  //     pin=0.95 → overall≈0.964 → scrollDelta=(0.964×3.6−1)×vh≈2.47vh
  //   projects  enterLen=1 pinLen=3.8 total=4.8 enterRatio≈0.208
  //     pin=0.95 → overall≈0.960 → scrollDelta=(0.960×4.8−1)×vh≈3.61vh
  //   about     enterLen=1 pinLen=2.6 total=3.6 (same as services)  ≈2.47vh
  const sectionPinOffset: Record<string, number> = {
    services: 2.47,
    projects: 3.61,
    about:    2.47,
  };

  // Custom scroll handler — bypasses Next.js router for same-page hash links so
  // GSAP-pinned sections don't block navigation. Also immediately dispatches the
  // correct nav theme so colors update without waiting for IntersectionObserver.
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    if (!target) return;
    e.preventDefault();
    const navTheme = lightThemeSections.has(id) ? 'light' : 'dark';
    window.dispatchEvent(new CustomEvent('eksa-nav-theme', { detail: { theme: navTheme } }));
    const absTop = target.getBoundingClientRect().top + window.scrollY;
    const pinOffset = sectionPinOffset[id];
    const top = pinOffset !== undefined
      ? absTop + pinOffset * window.innerHeight   // land deep in pin phase, content visible
      : absTop - 112;                             // normal: subtract navbar height
    window.scrollTo({ top: Math.max(0, top), behavior: 'smooth' });
  };

  const alternateLocale = locales.find((value) => value !== locale) ?? locale;
  const isDark = theme !== 'light';
  const brandClass = isDark ? 'text-[#f3e7d2]' : 'text-[#111111]';
  const navClass = isDark ? 'text-white' : 'text-[#111111]';
  const localeButtonClass = isDark
    ? 'rounded-full border border-white/26 bg-white/8 px-4 py-2 text-[13px] font-medium leading-none text-white backdrop-blur-sm'
    : 'rounded-full border border-black/18 bg-black/5 px-4 py-2 text-[13px] font-medium leading-none text-[#111111] backdrop-blur-sm';
  const mobileButtonClass = isDark
    ? 'flex size-11 items-center justify-center rounded-full bg-[rgba(255,255,255,0.14)] text-white backdrop-blur-sm lg:hidden'
    : 'flex size-11 items-center justify-center rounded-full bg-black/10 text-[#111111] backdrop-blur-sm lg:hidden';
  const mobilePanelClass = isDark
    ? 'border-white/10 bg-[rgba(7,20,18,0.92)] text-[#efe2cc]'
    : 'border-black/10 bg-[rgba(244,239,231,0.94)] text-[#111111]';

  return (
    <header className="fixed inset-x-0 top-0 z-[9999]">
      <div className="relative mx-auto flex h-[58px] w-full max-w-[1920px] items-center justify-between px-5 md:px-[38px] lg:px-[75px]">
        <Link
          href={`/${locale}`}
        ...
        >
          <HeaderBrand
            className={`text-[3.4rem] md:text-[4rem] ${brandClass}`}
          />
        </Link>

        <nav
          className={`hidden items-center gap-[29px] text-[15px] font-medium leading-[18.75px] lg:flex ${navClass}`}
        >
          {navItems.map((item) => (
            <Link
              key={item.href}
        ...
        <button
          ...
          aria-controls="eksa-mobile-menu"
          className={mobileButtonClass}
          onClick={() => setMenuOpen((value) => !value)}
        >
          <span className="flex w-[18px] flex-col gap-[4px]">
            <span
              className={`h-[1.5px] w-full origin-center bg-current transition-transform duration-200 ${menuOpen ? 'translate-y-[5.5px] rotate-45' : ''}`}
            />
            <span
              className={`h-[1.5px] w-full bg-current transition-opacity duration-200 ${menuOpen ? 'opacity-0' : ''}`}
            />
            <span
              className={`h-[1.5px] w-full origin-center bg-current transition-transform duration-200 ${menuOpen ? '-translate-y-[5.5px] -rotate-45' : ''}`}
            />
      ...
      <div
        id="eksa-mobile-menu"
        className={`absolute inset-x-0 top-full lg:hidden ${menuOpen ? 'pointer-events-auto translate-y-0 opacity-100' : 'pointer-events-none -translate-y-2 opacity-0'}`}
      >
        <div className="mx-auto w-full max-w-[1920px] px-5 md:px-[38px] lg:px-[75px]">
          <div
            className={`mt-2 rounded-[2.2rem] border px-4 py-4 shadow-[0_1.6rem_4rem_rgba(0,0,0,0.12)] backdrop-blur-md transition duration-200 ${mobilePanelClass}`}
          >
            <nav className="grid gap-1 text-[15px] font-medium leading-[18.75px]">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="flex items-center justify-between rounded-full px-3 py-3 transition-colors hover:bg-black/5"
                  onClick={(e) => handleNavClick(e, item.href)}
                >
                  <span>{item.label}</span>
                  <NavArrow />
            ...
            <div className="mt-4 flex items-center gap-3">
              <Link
                href={`/${alternateLocale}`}
                className={isDark ? 'rounded-full border border-white/26 bg-white/8 px-4 py-2 text-[13px] font-medium leading-none text-white' : 'rounded-full border border-black/18 bg-black/5 px-4 py-2 text-[13px] font-medium leading-none text-[#111111]'}
                onClick={() => setMenuOpen(false)}
              >
                {alternateLocale.toUpperCase()}
              </Link>
...
function BrandOutlineSvg() {
  ...
  return (
    <svg
      viewBox={`0 0 ${metrics.boxWidth} ${metrics.boxHeight}`}
...
export function HomeHero({ locale, messages }: HomeHeroProps) {
  ...
  useEffect(() => {
    const handleThemeChange = (event: Event) => {
      const customEvent = event as CustomEvent<{ theme?: HeaderTheme }>;
      setHeaderTheme(customEvent.detail?.theme === 'dark' ? 'dark' : 'light');
    };
  ...
  useLayoutEffect(() => {
    ...
    const context = gsap.context(() => {
      ...
      const outlineText = outline?.querySelector(
        '[data-brand-outline]',
      ...
      const outlineLength = Math.max(
        outlineText.getComputedTextLength() * 6,
        1,
      );
      ...
      scrollTimeline.to(
        [house, cutoutHouse],
        { yPercent: -40, scale: 1.3, transformOrigin: '50% 100%', duration: 1 },
        0,
      );
      scrollTimeline.to(
        [smokeTop, cutoutSmoke],
        { yPercent: 0, duration: 1 },
        0,
      );
      scrollTimeline.to(
        [cloudLeft, cutoutCloudLeft],
        { xPercent: -15, duration: 1 },
        0,
      );
      scrollTimeline.to(
        [cloudRight, cutoutCloudRight],
        { xPercent: 15, duration: 1 },
        0,
      );
      scrollTimeline.to(content, { yPercent: 20, scale: 0.9, duration: 1 }, 0);
      scrollTimeline.to(content, { opacity: 0, duration: 0.2 }, 0);
      scrollTimeline.to(outline, { opacity: 1, duration: 0.01 }, 0.12);
      scrollTimeline.to(
        outlineText,
        {
          strokeDashoffset: outlineLength * 0.18,
          duration: 0.36,
          ease: 'none',
        },
        0.12,
      ...
      entryTimeline.from(
        title,
        { opacity: 0, yPercent: 10, duration: 1.2, ease: 'expo.out' },
        0,
      );
      entryTimeline.from(
        text,
        { opacity: 0, yPercent: 20, duration: 1.1, ease: 'expo.out' },
        0.4,
      );
      entryTimeline.from(
        back,
        { scale: 1.1, duration: 5, ease: 'expo.out' },
        0,
      );
      entryTimeline.from(
        cloudLeft,
        { yPercent: 50, duration: 3, ease: 'expo.out' },
        0,
      );
      entryTimeline.from(
        cloudRight,
        { yPercent: 100, duration: 4, ease: 'expo.out' },
        0.1,
      );
      entryTimeline.from(
        house.querySelector('img'),
        { opacity: 0, duration: 0.6 },
        0.2,
      );
      entryTimeline.from(
        house.querySelector('img'),
        { yPercent: 10, duration: 3, ease: 'expo.out' },
        0.2,
      );
      ...
      >
        <div className="sticky top-0 h-screen" style={{ zIndex: 0 }}>
          <div className="pointer-events-none absolute inset-0 overflow-hidden">
            ...
            <div
              ref={smokeTopRef}
              className="absolute bottom-0 left-0 right-0 z-[5] h-[45rem] translate-y-[70%] md:h-[62rem]"
            >
              <Image
                src={smokeImage}
                alt=""
            ...
            <div className="mx-auto w-full max-w-[1440px] px-5 text-center md:px-[38px] lg:px-[75px]">
              ...
              >
                <p>
                  ...
                  <span className="text-[rgba(21,23,23,.5)]">
                    {messages.hero.descriptionMuted}
                  </span>
                </p>
              </div>
            </div>
